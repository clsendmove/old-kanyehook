...
void CMenu::DrawMenu()
{
	...
	if (ImGui::Begin(" ", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoTitleBar))
	{
...
void CMenu::MenuAimbot()
{
	...
	if (BeginTable("AimbotTable", 3))
	{
		...
		if (TableColumnChild("AimbotCol3"))
		{
			...
			WSlider("Strafe Chance", &Vars::Aimbot::Projectile::StrafeChance.Value, 0.1f, 100.f, "%.1f");
			{
...
void CMenu::MenuHvH()
{
	...
	if (BeginTable("HvHTable", 1))
	{
		...
		if (TableColumnChild("HvHCol1"))
		{
			...
			WSlider("Warp Per Tick", &Vars::Misc::CL_Move::DTTicks.Value, 0, 24, "%.1f", 0);
...
void CMenu::Render(IDirect3DDevice9* pDevice)
{
	...
	{
		DrawMenu();
		AddDraggable("Ping", Vars::Visuals::OnScreenPing, true, true);
		AddDraggable("DT Bar", Vars::Visuals::DTIndicator, Vars::Misc::CL_Move::DTBarStyle.Value, true);
		AddDraggable("Crits", Vars::Visuals::IndicatorPos, Vars::CritHack::Indicators.Value, true);
