...
std::pair<float, float> CCritHack::critMultInfo(CBaseCombatWeapon* wep)
{
	...
	float observed_chance = (pWeapon->m_flObservedCritChance());
...
void CCritHack::Run(CUserCmd* pCmd)
{
	...
	if (*reinterpret_cast<float*>(pWeapon + 2652) > 0 && *reinterpret_cast<float*>(pWeapon + 2648) > 0)
		critmultx3 = pWeapon->GetSlot() == SLOT_MELEE ? 0.5f : Math::RemapValClamped(*reinterpret_cast<float*>(pWeapon + 2652) / *reinterpret_cast<float*>(pWeapon + 2648), 0.1f, 1.f, 1.f, 3.f);
	else
		critmultx3 = 1.f;
...
void CCritHack::Draw()
{
	...
	const int count = *reinterpret_cast<int*>(pWeapon + 0xA5C);
	const int seed = *reinterpret_cast<int*>(pWeapon + 0xB58);
	const float cost = getWithdrawMult(pWeapon);
	const int attempt = *reinterpret_cast<int*>(pWeapon + 0xA58);
	const std::wstring critcount = L"CritCount: " + std::to_wstring(count);
	const std::wstring critseed = L"m_nCritSeedRequests: " + std::to_wstring(seed);
	const std::wstring attempts = L"Tries: " + std::to_wstring(attempt);


	const int x = Vars::Visuals::IndicatorPos.c;
	int currentY = Vars::Visuals::IndicatorPos.y;

	static auto tf_weapon_criticals_bucket_cap = g_ConVars.FindVar("tf_weapon_criticals_bucket_cap");
	const float bucketCap = tf_weapon_criticals_bucket_cap->GetFloat();
	const std::wstring bucketstr = L"Bucket: " + std::to_wstring(static_cast<int>(pWeapon->m_crit_token_bucket())) + L"/" + std::to_wstring(static_cast<int>(bucketCap));

	g_Draw.String(FONT_INDICATORS, x, currentY + 12, { 255, 255, 255, 255 }, ALIGN_CENTERHORIZONTAL, bucketstr.c_str());
	g_Draw.String(FONT_INDICATORS, x, currentY + 24, { 255, 255, 255, 255 }, ALIGN_CENTERHORIZONTAL, critseed.c_str());
	g_Draw.String(FONT_INDICATORS, x, currentY + 36, { 255, 255, 255, 255 }, ALIGN_CENTERHORIZONTAL, critcount.c_str());
	g_Draw.String(FONT_INDICATORS, x, currentY + 48, { 255, 255, 255, 255 }, ALIGN_CENTERHORIZONTAL, attempts.c_str());
}

void CCritHack::Events(CGameEvent* event, const FNV1A_t uNameHash)
{
