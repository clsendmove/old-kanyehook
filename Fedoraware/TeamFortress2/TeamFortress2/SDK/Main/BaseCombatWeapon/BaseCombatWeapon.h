...
public: //Netvars
	...
	//NETVAR(ObservedCritChance, float, "DT_TFWeaponBase", "m_flObservedCritChance");
...
public: //Everything else, lol
	...
	__inline float GetChargeTime(CBaseEntity* Entity, CBaseCombatWeapon* pWeapon)
	{
		auto cur_time{ static_cast<float>(Entity->m_nTickBase()) * I::GlobalVars->interval_per_tick};
		auto charge_begin_time{ GetChargeBeginTime() };
		auto charge{ cur_time - charge_begin_time };

		bool Sticky = GetWeaponID() == TF_WEAPON_PIPEBOMBLAUNCHER;

		float MAX = Sticky ? 4.0f : 1.0f;

		float TIME = Math::RemapValClamped(charge, MAX, 0.0f, 0.0f, 100.0f);
	...
	__inline bool CalcIsAttackCritical()
	{
		static auto func = g_Pattern.Find(_(L"client.dll"), _(L"53 57 6A ? 68 ? ? ? ? 68 ? ? ? ? 6A ? 8B F9 E8 ? ? ? ? 50 E8 ? ? ? ? 8B D8 83 C4 ? 85 DB 0F 84"));
