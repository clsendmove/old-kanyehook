...
void LimitAngles(QAngle& angles)
{
	if (angles.x > 180)
		angles.x -= 360;
	if (angles.x < -180)
...
void FixSpread(CUserCmd* pCmd)
{
	Vector vForward, vRight, vUp, vSpread;
	QAngle qFinal;

	LimitAngles(pCmd->viewangles);

	int iSeed = pCmd->random_seed & 255;
	*I::RandomSeed = iSeed;

	float Rand1 = Math::RandFloat(0.0f, PI * 2.0f);
	float Rand2 = Math::RandFloat(0.0f, PI * 2.0f);
	float Rand3 = Math::RandFloat(0.0f, PI * 2.0f);
	float Rand4 = Math::RandFloat(0.0f,  PI * 2.0f);
	...
	Math::VectorAngles(vNewAngles, qFinal);

	LimitAngles(qFinal);

	pCmd->viewangles += pCmd->viewangles - qFinal;
	     pCmd->viewangles.x += RAD2DEG(std::atan(std::sqrt(Spread.x * Spread.x + Spread.y * Spread.y)));
	pCmd->viewangles.z = -RAD2DEG(std::atan2(Spread.x, Spread.y));
}
